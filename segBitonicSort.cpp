...
void segmentedBitonicSort(float* data, int* seg_id, int* seg_start, int n, int m)
{
    ...
    if(n <= 0 || m <= 0|| m>n)
    {
        cout<<"Input error!Accurate conditions:n>m>0"<<endl;
        return;
    }
    ...
    {
        int len=1;
        int cur_len=seg_start[d+1]-seg_start[d];
        int start_point = seg_start[d] - seg_start[0];
                ...
                for (int step0 = step>>1; step0 >0; step0 >>= 1)// 2
                {
                    for (int j = 0; j < step; j += step0<<1)//3
                    {
                        for (int k = 0; k < step0; ++k)//4
                        {
                            ...
                            {
                                temp2-=cur_len;
                                int temp3=seg_start[d]-seg_start[0];
